...
class XAPI extends Listener_1.Listener {
    constructor({ accountId, password, type, appName = undefined, host = undefined, rateLimit = undefined, logger = new logger4_1.EmptyLogger(), safe = undefined, subscribeTrades = undefined }) {
                ...
                if (this.Socket.status === __1.ConnectionStatus.CONNECTED) {
                    if (this.isReady) {
                        this.Stream.ping().catch(e => {
                            Log_1.Log.error('Stream: ping request failed (XAPI.ts:172)');
                        });
        ...
        this.Socket.listen.login((data, time, transaction) => {
            ...
            if (this.isReady) {
                this.Stream.ping().catch(e => {
                    Log_1.Log.error('Stream: ping request failed (XAPI.ts: 208)');
                });
                this.Socket.send.getTrades(true).catch().then(() => {
                    if (this.isReady) {
                        this.callListener(Enum_1.Listeners.xapi_onReady);
        ...
        this.Socket.listen.getTrades((data, time, transaction) => {
            ...
            if (sent !== null && sent.elapsedMs() < 1000) {
                ...
                Log_1.Log.info('getTrades transaction (' + transaction.transactionId + ') is ignored');
            }
        });
        ...
        this.onReady(() => {
            ...
            if (this.isSubscribeTrades) {
                this.Stream.subscribe.getTrades().catch(e => {
                    Log_1.Log.error('Stream: getTrades request failed (XAPI.ts:312)');
                });
                this.Stream.subscribe.getTradeStatus().catch(e => {
                    Log_1.Log.error('Stream: getTrades request failed (XAPI.ts:315)');
                });
            }
            this.timer.interval.push(setInterval(() => {
                if (this.Socket.status === __1.ConnectionStatus.CONNECTED
                    ...
                    this.Socket.ping().catch(e => {
                        Log_1.Log.error('Socket: ping request failed (XAPI.ts:322)');
                    });
                }
                if (this.Stream.status === __1.ConnectionStatus.CONNECTED
                    && !this.Stream.isQueueContains('ping')) {
                    this.Stream.ping().catch(e => {
                        Log_1.Log.error('Stream: ping request failed (XAPI.ts:328)');
                    });
                }
                ...
                this.timer.timeout.push(setTimeout(() => {
                    if (this.Socket.status === __1.ConnectionStatus.CONNECTED
                        ...
                        this.Socket.send.getServerTime().catch(e => {
                            Log_1.Log.error('Socket: getServerTime request failed (XAPI.ts:337)');
                        });
                    }
                }, 1000));
                if (this.isSubscribeTrades) {
                    this.timer.timeout.push(setTimeout(() => {
                        if (this.Socket.status === __1.ConnectionStatus.CONNECTED
                            ...
                            this.Socket.send.getTrades(true).catch(e => {
                                Log_1.Log.error('Socket: getTrades request failed (XAPI.ts:346)');
                            });
                        }
                    }, 2000));
                }
            ...
            if (this.isSubscribeTrades) {
                this.timer.interval.push(setInterval(() => {
                    this.Stream.subscribe.getTrades().catch(e => {
                        Log_1.Log.error('Stream: getTrades request failed (XAPI.ts:363)');
                    });
                    this.Stream.subscribe.getTradeStatus().catch(e => {
                        Log_1.Log.error('Stream: getTrades request failed (XAPI.ts:366)');
                    });
                }, 60000));
            }
            this.timer.interval.push(setInterval(() => {
    ...
    getAccountCurrencyValue(anotherCurrency) {
        return Enum_1.Currency2Pair[anotherCurrency] === undefined
            ...
            : this.Socket.send.getSymbol(Enum_1.Currency2Pair[anotherCurrency]).then(({ returnData }) => {
                return this.Socket.send.getProfitCalculation(1, __1.CMD_FIELD.BUY, 0, Enum_1.Currency2Pair[anotherCurrency], 1).then(({ returnData: returnData2 }) => {
    ...
    onReady(callBack, key = null) {
        ...
        this.addListener(Enum_1.Listeners.xapi_onReady, callBack, key);
    }
    onReject(callBack, key = null) {
        this.addListener(Enum_1.Listeners.xapi_onReject, callBack, key);
    }
    ...
    onCreatePosition(callBack, key = null) {
        this.addListener(Enum_1.Listeners.xapi_onCreatePosition, callBack, key);
    }
    onDeletePosition(callBack, key = null) {
        this.addListener(Enum_1.Listeners.xapi_onDeletePosition, callBack, key);
    }
    onChangePosition(callBack, key = null) {
        this.addListener(Enum_1.Listeners.xapi_onChangePosition, callBack, key);
    }
    onPendingPosition(callBack, key = null) {
        this.addListener(Enum_1.Listeners.xapi_onPendingPosition, callBack, key);
    }
}
exports.XAPI = XAPI;
